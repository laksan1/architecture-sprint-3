...
5. Домен **SupportService**
   ...
   **Описание:**
   Этот домен отвечает за техподдержку системы, обработку запросов пользователей и управление обращениями по различным вопросам, связанным с работой экосистемы.

   **Границы контекста:**

   - Регистрация обращений пользователей
   - Обслуживание заявок пользователей
   - Классификация заявок (например, настройка оборудования, сбои, консультации)
   - Автоматическая маршрутизация обращений к соответствующим специалистам
   - Хранение истории обращений и их статусов
   - Оценка качества поддержки (сбор фидбэка от пользователей)
   - Интеграция с NotificationService для уведомления пользователей о статусе их заявок
   - Интеграция с AnalyticsService для анализа частоты и типов обращений

6. Домен **NotificationService**

   **Описание:**
   Этот домен отвечает за отправку уведомлений пользователям и техподдержке о важных событиях в системе, а также за управление каналами доставки уведомлений.

   **Границы контекста:**

   - Отправка уведомлений о подключении новых устройств
   - Отправка уведомлений об изменении различных событий (например, срабатывание датчиков, отключение света, ошибки в системе)
   - Уведомления о завершении или статусе заявок из SupportService
   - Гибкая настройка типов уведомлений пользователями (push-уведомления, email, SMS)
   - Логирование отправленных уведомлений и возможность их просмотра пользователями
   - Интеграция с UserManagementService для управления предпочтениями пользователей
   - Интеграция с AnalyticsService для анализа эффективности уведомлений (например, процент прочитанных уведомлений)

7. Домен **AnalyticsService**

   **Описание:**
   Этот домен отвечает за сбор, обработку и анализ данных системы. Он использует ClickHouse DB для хранения больших объемов данных и их быстрой аналитики.

   **Границы контекста:**

   - Сбор аналитики по всем приборам (отопление, освещение, безопасность)
   - Анализ потребления энергии и эффективности работы устройств
   - Построение пользовательских отчетов и дашбордов
   - Хранение и обработка исторических данных для долгосрочного анализа
   - Генерация прогнозов (например, предсказание поломок устройств на основе их поведения)
   - Интеграция с SupportService для анализа частоты и причин обращений в техподдержку
   - Интеграция с NotificationService для отправки аналитических уведомлений (например, превышение порогового значения температуры)
   - Поддержка API для интеграции с BI-системами и внешними аналитическими инструментами
...
![Скриншот](./diagrams/task_1/images/task_1_c4_context.png)
...
![Скриншот](./diagrams/task_2/images/task_2_c4_containers.png)

**Диаграмма компонентов (Components)**

[C4 Диаграмма компонентов](./diagrams/task_2/task_2_c4_components.puml)

![Скриншот](./diagrams/task_2/images/task_2_c4_components.png)

**Диаграмма кода (Code)**

[C4 Диаграмма кода](./diagrams/task_2/task_2_c4_code.puml)

![Скриншот](./diagrams/task_2/images/task_2_c4_code.png)

# Задание 3. Разработка ER-диаграммы

[ERD (Entity Relationship Diagram) ](./diagrams/task_3/er-diagram.puml)

![Скриншот](./diagrams/task_3/images/er-diagram.png)
...
⚠️ Проект со swagger не запускается, сделаны моковые файлы.

Был сделан моковый Controller [DeviceController](./diagrams/task_4/DeviceController.java) , который реализует 5 эндпоинтов.
...
![Скриншот](./diagrams/task_4/images/example.producer_consumer.png)
...
2. Yaml файл для документации асинхронных запросов [example.asyncAPI.yaml](./diagrams/task_4/examples/example.asyncAPI.json)
